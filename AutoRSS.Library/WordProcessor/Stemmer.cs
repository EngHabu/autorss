...
namespace AutoRSS.Library.WordProcessor
{
    ...
    public class Stemmer
    {
        public string Stem(string word)
        {
            ...
            if (word.IndexOf("-") > 0)
            {
                first = word.Substring(0, word.IndexOf("-") + 1); // Grabs hyphen too
        ...
        public string StemParagraph(string paragraph)
        {
            string[] words = BreakParagraph(paragraph);
         ...
         */
        private ArrayList stem_list(object p_words)
        {
            string[] newwords;
     ...
     */
        private string Step1(string p_word)
        {
            // Step 1a
            ...
            {
                if (Regex.IsMatch(p_word, ".*([aeiou]|[^aeiou]y).*(ed|ing)$"))
                { // vowel in stem
                    // Strip '-ed' or '-ing'
            ...
            if (Regex.IsMatch(p_word, "([aeiou]|[^aeiou]y).*y$"))
            { // vowel in stem
